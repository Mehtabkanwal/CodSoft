...
def player_choice(player_input):
    ...
    computer_choice_label.config(text = 'Computer Choice : ' + computer_input[0])

    if(player_input == computer_input):
        winner_label.config(text = "Tie")
    elif((player_input[1] - computer_input[1]) % 3 == 1):
        player_score += 1
        winner_label.config(text="You have Won!!!")
        player_score_label.config(text = 'Your Score : ' + str(player_score))
    else:
        computer_score += 1
        winner_label.config(text="Computer has Won!!!")
...
winner_label = Label(text = "Let's Begin the Game...", fg = 'green', font = font.Font(size = 13), pady = 8)
winner_label.pack()
